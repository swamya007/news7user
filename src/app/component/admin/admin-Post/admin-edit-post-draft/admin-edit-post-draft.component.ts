...
@Component({
  selector: 'app-admin-edit-post-draft',
  templateUrl: './admin-edit-post-draft.component.html',
  styleUrls: ['./admin-edit-post-draft.component.css']
})
export class AdminEditPostDraftComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.draft_id = Number(routeParams.get('draft_id'));
  ...
  getalldraftsbypost() {
    this.post.getdraftdetailsbypost(this.pid,this.draft_id,this.currentuser.customer_id).subscribe((res: any) => {
      if (res.code == 'success') {
        var data = res.body;
  ...
  saveas_draft() {
    ...
    console.log(this.pid,'king')
    this.updatepost.post_id=this.pid;
    alert(this.updatepost.post_id)
    this.updatepost.createdby = this.currentuser.user_id;
