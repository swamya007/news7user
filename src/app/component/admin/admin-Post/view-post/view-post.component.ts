...
export class ViewPostComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var years = Array.from(Array(new Date().getFullYear() - 2015), (_, i) =>
      (i + 2016).toString()
    );
    this.year = years.map((elem) => ({
      name: elem,
      val: parseInt(elem),
    }));
  }
  ...
  postbyauthor() {
    this.authorsearch = this.currentuser.user_id;
    this.getallpost();
  }
  ...
  searchByValue(type: any) {
    ...
    if (valuefor) {
      this.post
        .getPostBySearchVal(type, valuefor, this.currentuser.customer_id)
        .subscribe(
          (res: any) => {
            if (res.code == 'success') {
              var data = res.body;
    ...
    this.Categorydata.getAllCategory(
      ...
      (res: any) => {
        if (res.code == 'success') {
          ...
          if (
            this.updatepost.category !== null &&
            this.updatepost.category !== undefined
          ) {
            let arr: any = [];
  ...
  searchPost() {
    if (this.searchval) {
      this.post
        .getpostall(this.post_id, this.searchval, this.currentuser.customer_id)
        .subscribe(
          (res: any) => {
            if (res.code == 'success') {
              var data = res.body;
  ...
  clearAll() {
    ...
    obj.post_val = this.searchval;
    obj.category_id = this.categorysearch;
    obj.year = this.yearSearch;
    obj.month = this.monthSearch;
    obj.author = this.authorsearch;
    obj.date_val = this.dateSearch;

    this.post.postPagination15(obj).subscribe(
      (res: any) => {
        // this.post.postPagination(1, this.currentuser.customer_id, this.searchval, this.categorysearch, this.yearSearch, this.monthSearch, this.authorsearch, this.dateSearch).subscribe((res: any) => {
        if (res.code == 'success') {
          var data = res.body;
          this.postarr = data.map((dt: any) => JSON.parse(dt));
          this.totalNumber = this.postarr[0].total_counts;
          this.spinnerService.hide();
        } else {
          this.p = 1;
          this.postarr = [];
  ...
  getallpost() {
    ...
    this.post.postPagination(obj).subscribe(
      (res: any) => {
        // this.post.postPagination(1, this.currentuser.customer_id, this.searchval, this.categorysearch, this.yearSearch, this.monthSearch, this.authorsearch, this.dateSearch).subscribe((res: any) => {
        if (res.code == 'success') {
          var data = res.body;
  ...
  gty(page: any) {
    ...
    this.post.postPagination15(obj).subscribe(
      (res: any) => {
        // this.post.postPagination(page, this.currentuser.customer_id, this.searchval,this.categorysearch, this.yearSearch,this.monthSearch, this.authorsearch, this.dateSearch).subscribe((res: any) => {
        if (res.code == 'success') {
          var data = res.body;
          this.postarr = data.map((dt: any) => JSON.parse(dt));
          this.p = page;
  ...
  postDelete(id: any) {
    var funct = 'POST';
    this.masterService
      .bulkDeletion(funct, id, 0, this.currentuser.customer_id)
      .subscribe((res) => {
        if (res.code === 'success') {
      ...
      .subscribe(
        (res: any) => {
          if (res.code == 'success') {
            ...
            this.updatepost.immidiate_publish =
              this.updatepost.post_immidiate_publish;
  ...
  getalltag() {
    this.viewstag
      .getalltag(this.tagid, this.tag_name, this.currentuser.customer_id)
      .subscribe(
        (res: any) => {
          if (res.code == 'success') {
            var data = res.body;
            ...
            if (
              this.updatepost.tags !== null &&
              this.updatepost.tags !== undefined
            ) {
              let arr: any = [];
  ...
  modifypost() {
    ...
    if (
      this.updatepost.category !== null &&
      this.updatepost.category !== undefined &&
      this.updatepost.category !== ''
    ) {
      var result = this.updatepost.category
    ...
    if (
      this.updatepost.tags !== null &&
      this.updatepost.tags !== undefined &&
      this.updatepost.tags !== ''
    ) {
      var result = this.updatepost.tags
        .map(function (val: any) {
          return val.tag_name;
        })
        .join(',');
    ...
    if (this.updatepost.post_date) {
      if (this.updatepost.post_time) {
        this.updatepost.post_date =
          this.updatepost.post_date + ' ' + this.updatepost.post_time;
      } else {
    ...
    setTimeout(() => {
      ...
      this.post.addpost(this.updatepost).subscribe(
        (res: any) => {
          if (res.code == 'success') {
            this.spinnerService.hide();
            ...
            if (
              this.updatepost.addtags !== null &&
              this.updatepost.addtags !== undefined
            ) {
              let arr: any = [];
  ...
  redirectfn() {
    ...
    for (let i = 0; i < this.postarr.length; i++) {
      if (this.postarr[i].selected) this.checkedList.push(this.postarr[i].id);
    }
  ...
  addFeaturedsilder() {
    ...
    this.masterService
      .addtoslider(this.checkedList?.join(','), this.currentuser.customer_id)
      .subscribe(
        (res: any) => {
          this.checkedList = [];
          if (res.code === 'success') {
  ...
  deletetours() {
    ...
    this.masterService
      .bulkDeletion(funct, this.checkedList, 0, this.currentuser.customer_id)
      .subscribe(
        (res: any) => {
          this.checkedList = [];
          if (res.code === 'success') {
  ...
  alldata() {
    this.router.navigate(['/admin/post/view/all']);
  }
  ...
  addtrending() {
    ...
    this.masterService
      .addtotrending(this.checkedList?.join(','), this.currentuser.customer_id)
      .subscribe(
        (res: any) => {
          this.checkedList = [];
          if (res.code === 'success') {
