...
export class ImageCropperComponent implements OnChanges {
  ...
  aspectRatio = 1;
  @Input('aspectRatio')
  set setAspectRatio(value: number) {
    this.aspectRatio = value || 1;
  ...
  private transformBase64(exifOrientation: number): void {
    ...
    if (this.originalBase64) {
      ExifUtils.transformBase64BasedOnExifRotation(this.originalBase64, exifOrientation)
  ...
  crop() {
    ...
    if (displayedImage && this.originalImage != null) {
      ...
      if (ctx) {
        ...
        cropCanvas.toBlob(
          (fileImage: any) => this.imageCroppedFile.emit(fileImage),
          `image/${this.format}`,
          quality,
        );
      }
